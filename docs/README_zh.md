...
将自然语言转换为 SQL 查询语句。采用六阶段 NL2SQL 流水线架构：

1. **Schema Linking** — 智能筛选与问题相关的表/列
2. **Prompt Engineering** — 算子注册表、few-shot 示例、领域知识注入
3. **SQL Generation** — 调用 LLM 生成 SQL
4. **Validation & Self-Correction** — 语法校验 + LLM 自动修正（最多重试 N 次）
5. **Candidate Selection** — 多候选投票（预留）
6. **Security Check** — SQL 注入防护、危险操作过滤、自动补 LIMIT
...
-- 带增强参数
SELECT ai_query('minimax', '找出高价值订单', get_schema_info_enhanced(),
    auto_correct := true,
    max_retries := 2,
    read_only := true,
    max_limit := 1000
...
获取增强版 schema 信息，以 DDL `CREATE TABLE` 格式输出，包含主键、外键、列注释和示例值。

### get_relevant_schema(model_name, question) → text
基于 LLM 智能筛选与问题相关的表，返回精简的 schema 描述。
...
│   └── operators/                     # 算子子模块
│       ├── ai_filter.py               # 语义过滤
│       ├── ai_query/                  # NL2SQL 六阶段流水线
│       │   ├── __init__.py
│       │   ├── core.py                # 主入口
│       │   ├── schema_linking.py      # Schema 链接
│       │   ├── prompt_builder.py      # 提示词构建
│       │   ├── validator.py           # SQL 验证与自修正
│       │   └── security.py            # 安全检查
│       ├── ai_image.py                # 图片算子
│       ├── ai_audio.py                # 音频算子
│       └── batch.py                   # 批量处理算子
├── tests/
│   └── test_ai_query/                 # ai_query 单元测试 (133 tests)
├── sql/
│   ├── pg_semantic_operators--1.0.sql # SQL 扩展定义
│   └── test.sql                       # 测试脚本
├── docs/
│   ├── ai_query_improvement_plan.md   # ai_query 改进计划
│   └── README_zh.md                   # 中文文档
